...
inline double orbit_velocity(const double & radius)
{
...
inline void accelerate(std::vector<Star> & cluster, double dt)
{
	...
	for(unsigned int i=0; i<n-1;i++)
	{
		for(unsigned int j=i+1;j<n;j++)
		{
			...
			cluster[i].xacc = cluster[i].xacc - G*cluster[j].mass*xvec*pow(r,-3);
			cluster[i].yacc = cluster[i].yacc - G*cluster[j].mass*yvec*pow(r,-3);
			cluster[j].xacc = cluster[j].xacc + G*cluster[i].mass*xvec*pow(r,-3);
			cluster[j].yacc = cluster[j].yacc + G*cluster[i].mass*yvec*pow(r,-3);
		}
	}
	//std::cout << cluster[1].xacc;
	#pragma omp parallel for
	for(unsigned int i = 0; i < n ;i++)
	{
		cluster[i].xpos = cluster[i].xpos + cluster[i].xvel*dt;
		cluster[i].ypos = cluster[i].ypos + cluster[i].yvel*dt;
		cluster[i].xvel = cluster[i].xvel + cluster[i].xacc*dt;
		cluster[i].yvel = cluster[i].yvel + cluster[i].yacc*dt;
		cluster[i].xacc = 0.;
		cluster[i].yacc = 0.;
	}
}
